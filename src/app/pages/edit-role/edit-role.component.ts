...
@Component({
  ...
  standalone: true,
  imports: [ReactiveFormsModule,MatFormFieldModule,CommonModule,RoleDetailsFormComponent, MatInputModule, MatButtonModule],
...
export class EditRoleComponent implements OnInit{
  ...
  form:FormGroup = new FormGroup({
    role_name: new FormControl([{value:'',disabled:true},Validators.required]),
  ...
  async ngOnInit(){
    this.role_permissions = await this.contentService.getRolePermissions(this.role_info.role_id)
  ...
  async buildNestedCheckBox(){
    ...
    for(let mod of module_list){
      ...
      permissions.forEach((element:any) => {
        ...
        child_values.push({pid: element.permission_id, action: element.module_action, checked:isActionChecked})
        let child_checked:FormControl = new FormControl<boolean>(isActionChecked)
  ...
  async onSubmit(){
    ...
    const resp = await this.contentService.editRolePermissions(this.role_info.role_id,permissions);
